...
def generate_and_run( refine, n_particles, timestep, elastic_timestep, interpolator, reps ):
  prmfile = open("viscoelastic_stress_build-up_simple_shear_particles_dtc_isnot_dte.prm", "r")
  base_label = "ve_build-up_simple_shear_particles_"
  ...
  new_prmfile = "viscoelastic_stress_build-up_simple_shear_particles_" + label + ".prm"
  ...
  for l in prmfile.readlines():
    ...
    elif 'Use old velocities instead of current' in l and old_vel == True:
      outfile.write('    set Use old velocities instead of current for rotation = true \n' )
