...
for name in names: 
  ...
  ax1.scatter(counter,beam_depth[-1],label=None,color=colors[counter],linestyle=linestyles[counter],marker=markers[counter])
  ax1.text(counter,beam_depth[-1]-3,r'{0:.0f}'.format(beam_depth[-1]),fontsize=9,color=colors[counter],bbox=dict(facecolor='white', edgecolor='none',boxstyle='square,pad=0.'))
  beam_depth_50ky = beam_depth[np.where(time == 50000)][0]
  ax2.scatter(counter,beam_depth_50ky,label=labels[counter],color=colors[counter],linestyle=linestyles[counter],marker=markers[counter])
  ax2.text(counter,beam_depth_50ky-1.5,r'{0:.0f}'.format(beam_depth_50ky),fontsize=9,color=colors[counter])
...
ax1.hlines(2825.0,-50000,50000,color='black',linestyle='dotted',label=None,linewidth=1)
ax1.hlines(2806.25,-50000,50000,color='black',linestyle='dotted',label=None,linewidth=1)
...
ax2.legend(loc='lower left',ncol=2,handlelength=2,fontsize=8)
# Grid and tickes
...
ax1.set_title("Fields, htan10m, Newton, AMG, geometric, dt500", fontsize=6)
...
# Save as pdf
filename = '5_viscoelastic_bending_beam_dte_isnot_dtc_main_maxdeptherror_dh.png'
