...
colors = [color1, color1, color1, color1, color1, color4, color4, color4, color4, color4]
# Set the line styles
...
for name in names: 
  ...
  ax1.text(id_sim+0.1,beam_depth[-1]-3,r'{0:.0f}'.format(beam_depth[-1]),fontsize=9,color=colors[counter],bbox=dict(facecolor='white', edgecolor='none',boxstyle='square,pad=0.'))
  ...
  ax2.text(id_sim+0.1,beam_depth_50ky-1.5,r'{0:.0f}'.format(beam_depth_50ky),fontsize=9,color=colors[counter])
...
ax1.annotate("t = 350 ky", xytext=(-0.35,2825), xy=(1.5, 2815),ha='left',va='center')
ax1.text(-0.1,2800,r"$\mathrm{z_{max}(t_{0})}$",va='center',ha='left',bbox=dict(facecolor='white', edgecolor='none'))
ax2.annotate("t = 50 ky", xytext=(4.35,3055), xy=(1, 3055),ha='right',va='center')
...
ax1.set_title("Particles, htan10m, Newton, pw0, geometric, LLS/QLS, IGR2, IAR1, np4", fontsize=6)
...
ax1.set_ylim(2840,2795) # m
ax2.set_xlim(-0.5,4.5) # -
ax2.set_ylim(3095,3050) # m
...
# Save as pdf
filename = '5_viscoelastic_bending_beam_particles_dte_isnot_dtc_main_fieldprop_maxdeptherror_dt.png'
