...
for name in names: 
  ...
  ax[1].plot(counter,beam_depth[-1] - 2812.5,label=labels[counter],color=colors[counter],linestyle=linestyles[counter],marker=markers[counter])
  
  counter += 1

# Plot horizontal line at initial depth
ax[0].hlines(2812.5,-50000,50000,color='black',linestyle='dashed',label='original max depth',linewidth=1)
...
ax[0].set_xticks([])
ax[1].set_xticks([])
ax[0].set_title("Particles, htan10m, Newton, pw0, geometric, dt500, IGR2, IAR1, np4"

# Ranges of the axes
ax[0].set_xlim(-0.5,4.5) # -
ax[0].set_ylim(2700,2800) # m
ax[1].set_xlim(-0.5,4.5) # -
ax[1].set_ylim(-50,50) # m

# Add labels a) and b)
ax[0].text(-0.9,2750,"a)")
ax[1].text(-0.9,50,"b)")
