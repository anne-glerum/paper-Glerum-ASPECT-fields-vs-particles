...
names = [
         ...
         "ve_sheared_torsion_particles_interpolatorcell_average_dtc0.01_dte0.02_GR2_np4",
         "ve_sheared_torsion_particles_interpolatorcell_average_dtc0.005_dte0.02_GR2_np4",
         "ve_sheared_torsion_particles_interpolatorcell_average_dtc0.01_dte0.01_GR2_np4",
         "ve_sheared_torsion_particles_interpolatorcell_average_dtc0.005_dte0.01_GR2_np4",
         "ve_sheared_torsion_particles_interpolatorcell_average_dtc0.0025_dte0.01_GR2_np4",
        ]
tail = r"/statistics"

# The labels the graphs will get in the plot
labels = [
          'dtc = 0.02 s, dte = 0.02 s',
          'dtc = 0.01 s, dte = 0.02 s',
          'dtc = 0.005 s, dte = 0.02 s',
          'dtc = 0.01 s, dte = 0.01 s',
          'dtc = 0.005 s, dte = 0.01 s',
          'dtc = 0.0025 s, dte = 0.01 s',
         ]
...
for name in names: 
  ...
  # Read in the time and the average value of the xz component of the viscoelastic stress,
  ...
  ax[0].plot(time,stress_xz_ave,label=labels[counter],color=colors[counter],linestyle=linestyles[counter],marker=markers[counter],markevery=dmark)
  #axins.plot(time,stress_xz_ave,label=None,color=colors[counter],linestyle=linestyles[counter],marker=markers[counter],markevery=dmark)
  ax[1].plot(time,(stress_xz_ave-tau_xz_analytical(time))/tau_xz_analytical(time)*100.,label=labels[counter],color=colors[counter],linestyle=linestyles[counter],marker=markers[counter],markevery=dmark)

  for tau in stress_xz_ave:
    if tau > 15:
...
ax[1].legend(loc='upper left',ncol=3,handlelength=2)
# Grid and tickes
...
# Add labels a) and b)
ax[0].text(-0.055,15.5,"b)")
ax[1].text(-0.07,0.2,"b)")
...
# Save
plt.savefig('4_viscoelastic_sheared_torsion_particles_dtc_isnot_dte_dtcisnotdte.png',dpi=300)
