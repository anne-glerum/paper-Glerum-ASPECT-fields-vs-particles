...
names = [
         "surface_topo_diffusion_zero_flux__dt0.0005_GR0",
        ]
tail = r"/statistics"
...
def topo_analytical(x,time):
    ...
    kappa = 0.25
    if t <= 0:
      topo = amplitude * np.sin(np.PI*x/domain_width)
    else:
      sum = 0.
      for n in range(1, n_max+1):
        sum += np.cos(2.*n*np.PI*x/domain_width)
               * exp(-kappa*4.*n*n*np.PI*np.PI*time/(domain_width*domain_width))
               / ((4.*n*n)-1.)
               * (-4.*amplitude/np.PI);
        # a0=4A/pi --> a0/2=2A/pi
