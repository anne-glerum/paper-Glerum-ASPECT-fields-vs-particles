...
def generate_and_run( refine, n_particles, timestep, elastic_timestep, interpolator ):
  ...
  for l in prmfile.readlines():
    if 'Output directory' in l:
      outfile.write('set Output directory = /scratch/usr/bbpanneg/runs/fix_stresses_elasticity/paper_11072022/BM2/' + base_label + label + '\n')
  ...
  for l in jobscript.readlines():
    if '#SBATCH -J' in l:
      ...
      outjobscript.write('mpirun --map-by socket:pe=$OMP_NUM_THREADS /home/bbpanneg/software/aspect/build_release__fix_stresses_elasticity_dealii_9.4.0/aspect ' + new_prmfile + ' > /scratch/usr/bbpanneg/runs/fix_stresses_elasticity/paper_11072022/BM2/' + base_label + label + '/opla' )
    else:
  ...
  if not exists('/scratch/usr/bbpanneg/runs/fix_stresses_elasticity/paper_11072022/BM2/' + base_label + label):
    os.system('mkdir /scratch/usr/bbpanneg/runs/fix_stresses_elasticity/paper_11072022/BM2/' + base_label + label)
  
  # Submit simulation to run queue
  submit = 'sbatch '+ new_jobscript
  os.system(submit)

  print ('Model output in /scratch/usr/bbpanneg/runs/fix_stresses_elasticity/paper_11072022/BM2/' + base_label + label)

# Run the model at different refinement levels and timesteps
refinements = [0]
particles = [4]
elastic_timesteps = [125]
timesteps = [125]
