...
names = [
         ...
         "ve_sheared_torsion_particles_interpolatornearest_neighbor_dtc0.01_dte0.01_GR1_np4",
         "ve_sheared_torsion_particles_interpolatornearest_neighbor_dtc0.01_dte0.01_GR2_np4",
         "ve_sheared_torsion_particles_interpolatornearest_neighbor_dtc0.01_dte0.01_GR3_np4",
         "ve_sheared_torsion_particles_interpolatornearest_neighbor_dtc0.01_dte0.01_GR4_np4",
#         "ve_sheared_torsion_particles_interpolatorbilinear_least_squares_dtc0.01_dte0.01_GR1_np4",
#         "ve_sheared_torsion_particles_interpolatorquadratic_least_squares_dtc0.01_dte0.01_GR1_np4",
        ]
tail = r"/statistics"

# The labels the graphs will get in the plot
labels = [
          'cell average, dh = 0.5 m',
          'nearest neighbor, dh = 0.5 m',
          'nearest neighbor, dh = 0.25 m',
          'nearest neighbor, dh = 0.125 m',
          'nearest neighbor, dh = 0.06125 m',
...
colors = [color1, color2, color3, color4, color5, color6, color3, color5]
# Set the line styles
...
for name in names: 
  ...
  if 'particle' in name:
    time,stress_xz_min = np.genfromtxt(path, comments='#', usecols=(1,27), unpack=True)
  else:
    time,stress_xz_min = np.genfromtxt(path, comments='#', usecols=(1,40), unpack=True)

  # Plot the stress elements in Pa against time in s in
  # categorical batlow colors.
  ax[0].plot(time,stress_xz_min,label=labels[counter],color=colors[counter],linestyle=linestyles[counter],marker=markers[counter],markevery=dmark,fillstyle='none')
  ...
  ax[1].plot(time,(stress_xz_min-tau_xz_analytical(time))/tau_xz_analytical(time)*100.,label=None,color=colors[counter],linestyle=linestyles[counter],marker=markers[counter],markevery=dmark,fillstyle='none')
...
ax[0].legend(loc='upper left',handlelength=4,ncol=1)
...
# Save
plt.savefig('4_viscoelastic_sheared_torsion_particles_dtc_isnot_dte_interpolator_dh.png')    
