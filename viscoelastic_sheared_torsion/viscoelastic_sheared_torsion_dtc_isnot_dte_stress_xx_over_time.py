...
names = [
#         "ve_sheared_torsion_dtc0.02_dte0.02_GR1",
#         "ve_sheared_torsion_dtc0.01_dte0.02_GR1",
#         "ve_sheared_torsion_dtc0.005_dte0.02_GR1",
         "ve_sheared_torsion_dtc0.01_dte0.01_GR1",
#         "ve_sheared_torsion_dtc0.005_dte0.01_GR1",
#         "ve_sheared_torsion_dtc0.0025_dte0.01_GR1",
         "ST_test_2",
         "ST_test_3",
         "ST_test_4",
         "ST_test_particles",
...
labels = [
#          'dtc = 0.02 s, dte = 0.02 s, dh = 0.5 m',
#          'dtc = 0.01 s, dte = 0.02 s, dh = 0.5 m',
#          'dtc = 0.005 s, dte = 0.02 s, dh = 0.5 m',
          'dtc = 0.01 s, dte = 0.01 s, dh = 0.5 m, original',
          'dtc = 0.01 s, dte = 0.01 s, dh = 0.5 m',
          'dtc = 0.005 s, dte = 0.005 s, dh = 0.5 m',
          'dtc = 0.001 s, dte = 0.001 s, dh = 0.5 m',
#          'dtc = 0.005 s, dte = 0.01 s, dh = 0.5 m',
#          'dtc = 0.0025 s, dte = 0.01 s, dh = 0.5 m',
#          'dtc = 0.01 s, dte = 0.01 s, dh = 0.5 m',
          'dtc = 0.01 s, dte = 0.01 s, dh = 0.5 m, particles',
          'dtc = 0.005 s, dte = 0.005 s, dh = 0.5 m, particles',
          'dtc = 0.01 s, dte = 0.01 s, dh = 0.5 m, particles, no rot',
         ]
...
colors = [color1, color2, color3, color4, color5, color6, color1, color3, color5]
# Set the line styles
...
for name in names:
  ...
  if 'particle' in name:
    time,stress_xz_min,stress_xz_max,stress_xz_global = np.genfromtxt(path, comments='#', usecols=(1,27,28,29), unpack=True)
  else:
    time,stress_xz_min,stress_xz_max,stress_xz_global = np.genfromtxt(path, comments='#', usecols=(1,39,40,41), unpack=True)

  print ("xz min, xz_max, xz_global: ", stress_xz_min,stress_xz_max,stress_xz_global)

  # Plot the stress elements in Pa against time in s in
  # categorical batlow colors.
  #ax[0].plot(time,stress_xz_min,label=labels[counter],color=colors[counter],linestyle=linestyles[counter],marker=markers[counter],markevery=dmark)
  ...
  #axins.plot(time,stress_xz_min,label=None,color=colors[counter],linestyle=linestyles[counter],marker=markers[counter],markevery=dmark)
  #ax[1].plot(time,(stress_xz_min-tau_xz_analytical(time))/tau_xz_analytical(time)*100.,label=None,color=colors[counter],linestyle=linestyles[counter],marker=markers[counter],markevery=dmark)
...
#axins.plot(time,1e-6*tau_xz_analytical(time),label='analytical',color='black',linestyle='dashed')
...
ax[0].legend(loc='lower center',handlelength=4)
...
# Save
plt.savefig('4_viscoelastic_sheared_torsion_dtc_isnot_dte_dtcisdte_dtcisnotdte.png')
