...
# import colormap
Dir = r"C:/Users/Maaikew/Documents/VEP/Files for Anne Glerum/Colormaps/" # CHANGE directory
os.chdir(Dir)
...
for k in range(len(label_as)):
    exec('Dir = r"C:/Users/Maaikew/Documents/VEP/Files for Anne Glerum/Long 3D"\
        "/"+label_as[k]+"/"+label_as[k]+"" ') # CHANGE directory
...
Dir = r"C:/Users/Maaikew/Documents/VEP/Files for Anne Glerum/ABAQUS and TABOO/abaqus" # CHANGE directory
os.chdir(Dir) # change directory
...
Dir = r"C:/Users/Maaikew/Documents/VEP/Files for Anne Glerum/ABAQUS and TABOO/taboo" # CHANGE directory
os.chdir(Dir) # change directory
...
for k in range(len(label_as)):
    ...
    for l in range(3):
        ...
        if l != 0:
            plt.gca().tick_params(axis='y',labelleft=False)
        if l == 0:
            plt.ylabel('Vertical surface deformation [m]',fontsize=25)
        if l == 2:
            plt.legend(['t=10 yr','t=20 yr','t=30 yr','t=40 yr','t=50 yr','t=60 yr','t=70 yr','t=80 yr','t=90 yr','t=100 yr','t=110 yr','t=120 yr','t=130 yr','t=140 yr','t=150 yr','t=160 yr','t=170 yr','t=180 yr','t=190 yr','t=200 yr'],ncol=2,loc=1,fontsize=20)
        plt.grid(which='major')
        if l == 1:
            plt.xlabel('Horizontal distance from load center [km]',fontsize=25)
        plt.show()
...
for k in range(len(label_as)): 
    ...
    plt.ylabel('Maximum vertical surface deformation [m]',fontsize=25)
    plt.legend(['ASPECT','TABOO','Abaqus'],loc=3,fontsize=20)
    plt.grid(which='major')
    plt.show()
...
for k in range(len(label_as)):
    ...
    plt.ylim([-0.08,0.01])
    plt.xlabel('Time [yr]',fontsize=25)
    plt.ylabel('Vertical surface deformation rate [mm/yr]',fontsize=25)
    legend1 = plt.legend([plot_as_0,plot_t_0,plot_ab_0],['ASPECT','TABOO','Abaqus'],fontsize=20,loc=4,bbox_to_anchor=(1.0, 0.17))
    plt.legend([plot_as_0,plot_as_100,plot_as_250],['0 km','100 km','250 km'],loc=4,fontsize=20)
    ax = plt.gca()
    ax.tick_params(axis='both',labelsize=20)
    plt.grid(which='major')
