...
rc("pdf", fonttype=42)
rc("lines", linewidth=3, markersize=7)
...
names = [
         "ve_relaxation_particles_interpolatorcell_average_dtc500_dte500_GR0_g0",
         "ve_relaxation_particles_newIstress_interpolatorcell_average_dtc250_dte500_GR0_np4_g0",
         "ve_relaxation_particles_newIstress_interpolatorcell_average_dtc125_dte500_GR0_np4_g0",
         "ve_relaxation_particles_interpolatorcell_average_dtc250_dte250_GR0_g0",
         "ve_relaxation_particles_newIstress_interpolatorcell_average_dtc125_dte250_GR0_np4_g0",
        ]
tail = r"/statistics"
...
colors = [color2, color3, color4, color5, color6, color5, color6, "black", color3, color4, color5]
# Set the line styles
linestyles = ['solid', 'solid', 'solid', 'solid', 'solid', 'solid', 'solid', 'solid', 'dashdot', 'dashdot', 'dotted',  'dotted','dotted'] 
...
zoom_y_min = 9
zoom_y_max = 11
#axins = zoomed_inset_axes(ax[0], 6, loc = "upper center") # zoom = 3
...
for name in names: 
  ...
  ax[0].plot(time/1e3,stress_xx_min/1e6,label=labels[counter],color=colors[counter],linestyle=linestyles[counter],marker=markers[counter],markevery=20)
#  axins.plot(time/1e3,stress_xx_min/1e6,label=None,color=colors[counter],linestyle=linestyles[counter],marker=markers[counter],markevery=20)
  ax[1].plot(time/1e3,(stress_xx_min-(20e6*np.exp(-1e10*time*yr_in_secs/1e22)))/(20e6*np.exp(-1e10*time*yr_in_secs/1e22))*100.,label=labels[counter],color=colors[counter],linestyle=linestyles[counter],marker=markers[counter],markevery=20)
...
# Save as png
plt.savefig('1_viscoelastic_relaxation_particles_dte_isnot_dtc_dteisnotdtc.png')    
