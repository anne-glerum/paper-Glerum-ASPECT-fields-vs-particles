...
# Set up one plot showing the error with the analytical solution
# at different time steps with time step size.
...
for name in names: 
  ...
  # which is stored on the fields ve_stress_xx.
...
x = np.array([500,250,125,62.5,15.625])
def y(a,b):
  return a-b*x
...
ax[0].set_title(r"Error at specific model times with time step size")
# Grid and tickes
ax[0].grid(axis='x',color='0.95')
ax[0].grid(axis='y',color='0.95')
ax[0].set_xticks([15.625,62.5,125,250,500])
...
# Place legend
ax[0].legend(loc='upper right',ncol=1,handlelength=4)
ax[0].set_yticks([1e-2,1e-1,1e0,1e1])
# Ranges of the axes
ax[0].set_xlim(2e3,9e0) # yr
ax[0].set_ylim(1e-3,2e1) # %
# Save as png
